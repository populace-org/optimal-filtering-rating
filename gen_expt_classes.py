...
# nodes now have ([realization], worker_class)
...
# construct the lattice assuming all workers from class 0
...
# assumes only one worker class == 0
def add_next(node, d):
    global R, m, parents, children, depth, nodes_in_depth
    last_node = [[], 0]
    for i in range(R-1):
        last_node[0].append(0)
    last_node[0].append(m)
    last_node2 = [tuple(last_node[0]), last_node[1]]
    if node == last_node2:
        children[node] = []
        return
    for i in range(R-1):
        if node[0][i] > 0:
            new_node_list = [list(node[0]), node[1]]
            new_node_list[0][i] -= 1
            new_node_list[0][i+1] += 1
            new_node = [tuple(new_node_list[0]), new_node_list[1]]
...
def all_combinations(assigned, to_assign, curr_assign, curr_depth):
    global R, m, parents, children, depth, nodes_in_depth
    global num_bucketings, bucketings, rule, avg
...
def set_error(random_e):
    ...
    for j in range(max_workers):
        ...
        #two worker classes [good=0, bad=1]
        elif random_e == 3:
