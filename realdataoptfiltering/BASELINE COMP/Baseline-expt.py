...
def compute_algo(algo):
    ...
    if algo == "OPT":
        ...
        ##HERE DOING GENERAL EM!
        #em_bucketing2 = Return_em_bucketing(2)
        median_bucketing = Return_median_bucketing()
        temp = compute_likelihood(median_bucketing)

        likelihood[algo].append(temp/m)
        
        e_med = calc_e(median_bucketing)
        temp2 = emd_score(e_med, median_bucketing)
        score[algo].append(temp2)
...
def rte_load():
    ...
    with open(in_file, "r") as fp:
        for i, line in enumerate(fp):
            ...
            answers_full[i, j] = int(parts[3])

    max_workers_data = 164
    n = 800


def main2_rte(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global m
    # T.clear()  # true values of items
    ...
    for i in range(n):
        for j in worker_set[i]:
            temp = answers_full[i, j]

            temp_node[i][temp] += 1
            workers_answered[i].append(j)
            full_realization[i][j] = temp

    for i in range(n):
        item_M[i] = tuple(temp_node[i])
        lattice_M[item_M[i]] += 1
...
def temp_load():
    ...
    with open(in_file, "r") as fp:
        for i, line in enumerate(fp):
            ...
            i = item_map[item]
            j = worker_map[worker]
            T[i] = (int)(parts[4]) - 1  # these values are 1, 2
            workers_answered_full[i].append(j)
            answers_full[i, j] = int(parts[3]) - 1

    max_workers_data = 76
    n = 462


def main2_temp(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global m
    # T.clear()  # true values of items
...
def real_main2_trec(max_workers=max_workers_data):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global m
    # T.clear() #true values of items
...
def real_main2(max_workers, dataset):
    if dataset == "rte":
        main2_rte(max_workers)
    elif dataset == "temp":
        main2_temp(max_workers)
    elif dataset == "trec":
...
def main3():
    ...
    if dataset == "rte":
        ...
        m = 2
        # algo_range = ["OPT", "GEN_EM"]
        algo_range = ["OPT", "median"]
        max_workers = max_workers_data
    ...
    out_file = '{}.out.txt'.format(dataset)
