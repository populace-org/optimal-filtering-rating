...
def calc_e_full(f):
    ...
    e = {}
    for j in range(max_workers_data):
        for r1 in range(R):
            for r2 in range(R):
                e[j,(r1,r2)] = 0.0
...
#max_workers_data stores actually available worker pool
#max_workers stores the number used in the current experiment run


workers_answered_load = defaultdict(list)
full_realization_load = defaultdict(list)
max_workers_data = 0

def real_main_load():
    global n, T, item_M, lattice_M, workers_answered_load, full_realization_load

    file_data = open('IC_Data.txt','r')
    file_gold = open('IC_Gold.txt','r')

    workers_answered_load.clear()
    full_realization_load.clear()
    ...
    #pick m random answers for each item
    for line2 in file_data:
        for i in range(max_workers_data):
            #print i, 2*i
            temp = (int)(line2[2*i])
            #print temp
            if temp == 0:
                temp_node[0] += 1
                workers_answered_load[n].append(i)
                full_realization_load[n][i] = 0
            elif temp == 1:
                temp_node[1] += 1
                workers_answered_load[n].append(i)
                full_realization_load[n][i] = 1
            else:
...
def real_main2_IC(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    ...
    max_worker_set = random.sample(range(max_workers_data),max_workers)
    #pick m random answers for each item
    for line2 in file_data:
        #print line2,
        temp_node = [0,0]
        worker_set = random.sample(max_worker_set,m)
        #print worker_set
        #print line2
        for i in worker_set:
            #print i, 2*i
            ...
            if temp == 0:
                temp_node[0] += 1
                workers_answered[n].append(i)
                full_realization[n][i] = 0
            elif temp == 1:
                temp_node[1] += 1
                workers_answered[n].append(i)
                full_realization[n][i] = 1
            else:
                print "Error parsing data"
            #print temp_node
        #print temp_node
        item_M[n] = tuple(temp_node)
        lattice_M[item_M[n]] += 1
...
def real_main2_kdd(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    T.clear() #true values of items
    item_M.clear() #key = item index, value = realization tuple
    ...
    file_data = open('kdd2012-dataset/dataset.txt','r')
    ...
    max_worker_set = random.sample(range(max_workers_data),max_workers)
    #pick m random answers for each item
    worker_set = defaultdict(list)

    # j = items (columns), i = workers (rows)
    for j in range(n):
        worker_set[j] = random.sample(max_worker_set,m)
        #print worker_set
    ...
    for i, line2 in enumerate(file_data):
        answers = [int(float(x)) for x in line2.split()]
        for j in range(n):
            if i not in worker_set[j]:
                continue
            
            temp = answers[j]
            #print temp
            temp_node[j][temp] += 1
            workers_answered[j].append(i)
            full_realization[j][i] = temp
    
    for j in range(n):
        item_M[j] = tuple(temp_node[j])
        lattice_M[item_M[j]] += 1
...
workers_answered_full = {}
answers_full = {}
def trec_data_load():
    file_data = open('trec-rf10-crowd/trec-6.txt','r')
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global workers_answered_full, answers_full
    T.clear() #true values of items
    item_M.clear() #key = item index, value = realization tuple
    lattice_M.clear() #key = lattice node, value = number of items satisfying
    ...
    workers_answered_full = defaultdict(list)
    answers_full = {}
    ...
    for line in file_data:
        ...
        if n < i:
            n = i
        if max_workers_data < j:
            max_workers_data = j

        workers_answered_full[i].append(j)
        answers_full[i,j] = int(splits[3])

    n += 1
    max_workers_data += 1

def real_main2_trec(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global m
    # T.clear() #true values of items
    ...
    for j in range(n):
        worker_set[j] = random.sample(workers_answered_full[j],m)
        #print worker_set
    ...
    for j in range(n):
        for i in worker_set[j]:
            temp = answers_full[j,i]
            #print temp
            temp_node[j][temp] += 1
            workers_answered[j].append(i)
            full_realization[j][i] = temp
...
def adult_data_load_old():
    file_data = open('adult-website/adult_gold_common_workers.txt','r')
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global workers_answered_full, answers_full
    T.clear() #true values of items
...
def real_main2_adult_old(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global m
    # T.clear() #true values of items
...
common_worker_set = set()
def adult_data_load():
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global workers_answered_full, answers_full
    global common_worker_set
    T.clear() #true values of items
    item_M.clear() #key = item index, value = realization tuple
    lattice_M.clear() #key = lattice node, value = number of items satisfying

    file_in = open('adult-website/ratings.txt','r')
    ...
    n = num_items
    max_workers_data = num_workers

    workers_answered_full = workers_answering
    answers_full = answers
    T = gold

    print "n,max =", n, max_workers_data


def real_main2_adult(max_workers):
    global n, T, item_M, lattice_M, workers_answered, full_realization
    global max_workers_data
    global m
    # T.clear() #true values of items
    ...
    # max_worker_set = random.sample(range(max_workers_data),max_workers)
    # print max_workers_data, len(common_worker_set), max_workers
    temp_set = random.sample(\
        set(range(max_workers_data)) - common_worker_set, \
        max_workers - len(common_worker_set))
    max_worker_set = set(temp_set) | common_worker_set

    for j in range(n):
        worker_set[j] = random.sample(\
            max_worker_set & set(workers_answered_full[j]), m)
        #print worker_set
...
def real_main2(max_workers, dataset):
    if dataset == "IC":
        real_main2_IC(max_workers)
    elif dataset == "kdd":
        real_main2_kdd(max_workers)
    elif dataset == "trec":
        real_main2_trec(max_workers)        
    elif dataset == "adult":
        real_main2_adult(max_workers)  

#main3 --- for evaluating different bucketings
def main3():
    global R, m, parents, children, depth, nodes_in_depth, num_bucketings, bucketings, rule
    global n, T, item_M, lattice_M, s_rule, e_rule, s, e_gen, e_calc
    global max_workers, max_workers_data
    ...
    #m_range = [1,2,3,4,5,6]
    if R == 2: m_range = [2,3,4,5,6,7,8,9,10]
    #m_range = [1,2,3]
    #m_range = [3,4,5,6]
    ...
    if dataset == "IC":
        if not R == 2:
            print "Dataset, parameter mismatch"
            sys.exit()
        else:
            m_range = [2,3,4,5,6,7,8,9,10]
            ...
            #m = 2
            max_workers_data = 19
            max_worker_range = range(m,max_workers_data+1)
            #max_workers = max(m_range)
            max_workers = 19
            if R==3: m_range = [2,3,4,5,6]
        ...
        max_workers_data = 402
        max_worker_range = range(m,max_workers_data,40)
        max_workers = 6
        if R==3: m_range = [2,3,4,5,6]

    elif dataset == "trec":
        max_workers_data = 524
        trec_data_load()
        R = 3
        m_range = [1,2,3,4,5,6]
        algo_range = ["OPT","GEN_EM"]
        ...
        max_worker_range = range(10,max_workers_data+1,30)
        #max_workers = max(m_range)
        max_workers = 6
        ...
        max_worker_range = range(
            #len(common_worker_set),max_workers_data+1,30)
            250,max_workers_data+1,30)
        #max_workers = max(m_range)
        max_workers = 6        
    ...
    #for m in m_range:
        ...
        elif vary == "max_workers": num_trials = num_trials_base#num_trials_range[rd]
        rd += 1
        for i in range(num_trials):
            ...
            real_main2(max_workers, dataset)
            for algo in algo_range:
                compute_algo(algo)
    ...
    data_title = metric_func + ',' + \
        vary + ',' + \
        str(R) + ',' + \
        str(m) + ',' + \
        dataset + ',' + \
        str(max_workers)
    save_data = open(data_title,"a")
    save_data.write("-----------------------\n")
    save_data.write(data_title+"\n")
    save_data.write(str(num_trials_base)+" Trials"+"\n")
    save_data.write(str(m_range)+" m_range"+"\n")
    save_data.write(str(max_worker_range)+" max_worker_range"+"\n")
    save_data.write(str(n)+" "+str(max_workers_data)+"\n")
    save_data.write(str(mean_x)+"\n")
    save_data.write(str(mean_score)+"\n")
    save_data.write("\n")
    ...
    if R == 3 and metric_func == "dist_weighted":
        matplotlib.pyplot.savefig('rating-dist_wtd, vary '+vary+str(m)+'.png')
