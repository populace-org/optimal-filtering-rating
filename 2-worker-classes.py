...
w_class = {}  # worker id to class id


def set_worker_class(num_classes=2):
    global max_workers
    for j in range(max_workers):
        w_class[j] = random.choice([0, 1])


def total_depth():
    return max(nodes_in_depth.keys()) + 1


def construct_two_classes_lattice_small():
    global R, m, parents, children, depth, nodes_in_depth
    ...
    # children
    children[node0].append(node1)
    children[node1].append(node2)
    children[node1].append(node3)
    children[node1].append(node5)
    children[node2].append(node4)
    children[node3].append(node4)
    children[node4].append(node6)
    children[node4].append(node7)
    children[node5].append(node6)
    children[node6].append(node8)
    children[node7].append(node8)
    children[node8].append(node9)

    # parents
    parents[node1].append(node0)
    parents[node2].append(node1)
    parents[node3].append(node1)
    parents[node4].append(node2)
    parents[node4].append(node3)
    parents[node5].append(node1)
    parents[node6].append(node4)
    parents[node6].append(node5)
    parents[node7].append(node4)
    parents[node8].append(node7)
    parents[node8].append(node6)
    parents[node9].append(node8)
...
def set_error(random_e):
    ...
    for j in range(max_workers):
        ...
        #two worker classes [bad, good]
            ...
            # print "2 Worker classes generated"
            if R == 2 and w_class[j] == 0:
                e_gen[j,(0,0)] = random.uniform(0.9, 1.0)
                e_gen[j,(1,0)] = 1.0 - e_gen[j,(0,0)]
                e_gen[j,(1,1)] = random.uniform(0.9, 1.0)
                e_gen[j,(0,1)] = 1.0 - e_gen[j,(1,1)]
            elif R == 2 and w_class[j] == 1:
                e_gen[j,(0,0)] = random.uniform(0.6, 0.7)
                e_gen[j,(1,0)] = 1.0 - e_gen[j,(0,0)]
                e_gen[j,(1,1)] = random.uniform(0.6, 0.7)
                e_gen[j,(0,1)] = 1.0 - e_gen[j,(1,1)]
...
def generate_M():
    ...
    for i in range(n):
        ...
        for j in range(2):
            # worker class loop
            for r in range(R):
                total_responses[j * R + T[i]] += (float)(realization_list[j * R + r])
                e_calc[j, (r, T[i])] += (float)(realization_list[j * R + r])
    ...
    for j in range(2):
        ...
        for r1 in range(R):
            for r2 in range(R):
                if total_responses[j * R + r2] > 0.0:
                    e_calc[j, (r1,r2)] /= total_responses[j * R + r2]
                else: 
                    e_calc[j, (r1,r2)] = 0.0    
    return
    ...
    for node in lattice_M:
        ...
        for j in range(2):
            for r in range(R):
                ...
                #print "r = ",r
                total_responses[j * R + true_r] += \
                    lattice_M[node]*(float)(realization_list[j * R + r])
                e[j, (r,true_r)] += \
                    lattice_M[node]*(float)(realization_list[j * R + r])
    ...
    if e_calc_constraints == 1:
        ...
        for j in range(2):
            for r1 in range(R):
                for r2 in range(R):
                    if total_responses[j * R + r2] > 0.0:
                        e[j, (r1,r2)] /= total_responses[j * R + r2]
                    else:
...
def compute_likelihood(f,full=False):
    ...
    for node in lattice_M:
        ...
        for j in range(2):
            for r in range(R):
                if realization_list[j * R + r] > 0 and err[j, (r,true_r)]==0:
                    print "ERROR"
                elif err[j, (r,true_r)] > 0:
                    p_node += realization_list[j * R + r] * \
                        math.log(err[j, (r,true_r)])
...
def main1():
    ...
    #print "R = ",R,", m = ",m
    #construct_lattice()
    construct_two_classes_lattice_small()
...
def main3():
    ...
    #max_worker_range = [2,20,200,2000]
    ...
    #matplotlib.pyplot.legend(['OPT','EM(1)','GEN_EM','EM(3)','EM(*)'],prop={'size':18})
    #matplotlib.pyplot.legend(['OPT','EM(1)','EM(2)','EM(3)','EM(*)'],prop={'size':18})
    if R == 2 and vary == "max_workers": matplotlib.pyplot.legend(leg,prop={'size':18},loc='upper right')
    else: matplotlib.pyplot.legend(leg,prop={'size':18})
    ...
    all_out_file = '2-worker-classes.out.txt'
    with open(all_out_file, "a+") as fp:
        fp.write("---------------------")
        # params
        fp.write('{},{},{},{}\n'.format(vary, R, metric_func, num_trials_base))
        fp.write('{},{},{}\n'.format(m, max_workers, max_worker_range))
        # x
        fp.write('{}\n'.format(mean_x))
        for algo in algo_range:
            # like
